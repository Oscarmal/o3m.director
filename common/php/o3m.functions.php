...
function sanitizerSpace($param, $revert=false) {
#Sanitizes a url param
	if(!$revert){
	    $strip = array(" ",);
	    $clean = trim(str_replace($strip, "+", strip_tags($param)));
...
// Validación de grupos de acceso
	...
	switch ($usuario[id_grupo]){	
		...
		#Grupo 1 - Nivel2:
		case 34 : $filtro = $grupos[34]; break;
		#Grupo 1 - Nivel3:
		case 35 : $filtro = $grupos[35]; break;
		#Grupo 2 - Nivel2:	
...
function fecha_form($fecha,$hora=0){
	switch ($hora) {
		case 0: $hora = ' H:i:s'; break;
		case 1: $hora = ''; break;		
		default: $hora = ' H:i:s'; break;
	}
	$fecha = date_create_from_format('d/m/Y', $fecha);
	$fecha = date_format($fecha, 'Y-m-d'.$hora);
	return $fecha;
}
...
function dia($fecha){
	...
	$fecha = date_format($fecha, 'w');	
	return $dias[$fecha];
}

function Genera_Aleatorio($length,$uc,$lc,$n,$sc){
    $source = '';
...
function meses($id_mes,$txt=false){
	$mes = $id_mes;
	if(!$txt){
		...
		if (strtolower($id_mes)=="enero") 		$mes=1;
		if (strtolower($id_mes)=="febrero") 	$mes=2;
		if (strtolower($id_mes)=="marzo") 		$mes=3;
		if (strtolower($id_mes)=="abril") 		$mes=4;
		if (strtolower($id_mes)=="mayo") 		$mes=5;
		if (strtolower($id_mes)=="junio") 		$mes=6;
		if (strtolower($id_mes)=="julio") 		$mes=7;
		if (strtolower($id_mes)=="agosto") 		$mes=8;
		if (strtolower($id_mes)=="septiembre") 	$mes=9;
		if (strtolower($id_mes)=="octubre") 	$mes=10;
		if (strtolower($id_mes)=="noviembre") 	$mes=11;
		if (strtolower($id_mes)=="diciembre") 	$mes=12;
	}
	return $mes;
}

function o3mcontrolador($controlador=false, $params=array()){
// Valida la existencia del controlador soicitado, lo ejecuta y devuelve su contenido
	if(function_exists($controlador)){
		$result = call_user_func($controlador,$params);
	}else{
