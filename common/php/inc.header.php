...
$Path[chords]	= $Path[docs].$cfg[path_chords];
...
$Path[chordsurl]= $Path[docsurl].$cfg[path_chords];
...
require_once($Path[local].'o3m.paginador/o3m.paginador.class.php');
require_once($Path[local].'o3m.charts/o3m.charts.php');
require_once($Path[local].'o3m.guitarTabs/o3m.guitarTabs.class.php');
require_once($Path[local].'fpdf17/fpdf.php');
...
$usuario[id_perfil]			= $_SESSION[user]['id_perfil'];
...
$usuario[accesos][visible_submenu]	= $_SESSION[user]['accesos']['visible_submenu'];
$usuario[accesos][invisible_submenu]= $_SESSION[user]['accesos']['invisible_submenu'];
...
if($cfg[log_onoff] && $in[s]!=$parm[LOGIN]){
	...
	LogTxt($cfg[app_prefix].'_'.str_replace(' ','-',$usuario[empresa]),$usuario[id_usuario],$usuario[nombre],$usuario[usuario],$usuario[grupo],$Raiz[local],$params);
}	
#Online
